...
export default function UseEffectDidUpdate() {
  ...
  useEffect(() => {
    //chạy lần đầu tiên và chạy mỗi khi tham số thứ 2 ( dependency ) thay đổi
    if (provinceId !== "") {
      setArrDistrict(
        districts.filter((item) => item.provinceId === provinceId)
      );
    }
  }, [provinceId]); // shalow compare (so sánh đối với state là dữ liệu cơ sở - đối với referrence value thay đổi thì khi setState {...} || [...])
